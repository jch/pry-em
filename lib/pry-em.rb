EmCommands = Pry::CommandSet.new do
  ...
  create_command /\s*em\s*([0-9\.]*)\s*:(.*)/ do
    ...
    def process(timeout, source)
      ...
      $_em_pry_retval, $_em_pry_em_state = [nil, :waiting]
    ...
    def wait_for_deferrable(deferrable, timeout)
      ...
      if timeout
        EM::Timer.new(timeout) { $_em_pry_em_state = :timeout if waiting? }
      end

      [:callback, :errback].each do |method|
        begin
          deferrable.__send__ method do |*result|
            $_em_pry_em_state = method
            $_em_pry_retval = result.size > 1 ? result : result.first
          end
        rescue NoMethodError
          $_em_pry_retval = deferrable
          $_em_pry_em_state = :callback
          break
        end
      end

      sleep 0.01 until $_em_pry_em_state != :waiting

      raise "Timeout after #{timeout} seconds" if $_em_pry_em_state == :timeout

      # Use Pry's (admittedly nascent) handling for exceptions where possible.
      raise $_em_pry_retval if $_em_pry_em_state != :callback && Exception === $_em_pry_retval

      # TODO: This doesn't interact well with the pager.
      output.print "#{$_em_pry_em_state} " if $_em_pry_em_state != :callback

      $_em_pry_retval

    # If the main thread is interrupted we must ensure that the $_em_pry_em_state
    # is no-longer :waiting so that handle_unexpected_error can do the
    # right thing.
    ensure
      $_em_pry_em_state = :interrupted if waiting?
    end

    def waiting?
      $_em_pry_em_state == :waiting
    end
  end
end
